...
__global__ void blockedFW_Phase1(int *D, unsigned int round, unsigned int V) {
    ...
    for(int i = 0; i < BlockingFactor; i++) {
        ...
        /*shared_D[y][x] = (shared_D[y][x] > \
            shared_D[y][i] + shared_D[i][x]) ? \
            shared_D[y][i] + shared_D[i][x] : shared_D[y][x];

        shared_D[y + Half_BF][x] = (shared_D[y + Half_BF][x] > \
            shared_D[y + Half_BF][i] + shared_D[i][x]) ? \
            shared_D[y + Half_BF][i] + shared_D[i][x] : shared_D[y + Half_BF][x];

        shared_D[y][x + Half_BF] = (shared_D[y][x + Half_BF] > \
            shared_D[y][i] + shared_D[i][x + Half_BF]) ? \
            shared_D[y][i] + shared_D[i][x + Half_BF] : shared_D[y][x + Half_BF];

        shared_D[y + Half_BF][x + Half_BF] = (shared_D[y + Half_BF][x + Half_BF] > \
            shared_D[y + Half_BF][i] + shared_D[i][x + Half_BF]) ? \
            shared_D[y + Half_BF][i] + shared_D[i][x + Half_BF] : shared_D[y + Half_BF][x + Half_BF];*/
...
__global__ void blockedFW_Phase2(int *D, unsigned int round, unsigned int V) {
    ...
    for(int k = 0; k < BlockingFactor; k++) {
        ...
        /*
        shared_Pivot_Col[y][x] = (shared_Pivot_Col[y][x] > \
            shared_Pivot_Col[y][k] + shared_Pivot_Block[k][x]) ? \
                shared_Pivot_Col[y][k] + shared_Pivot_Block[k][x] : shared_Pivot_Col[y][x];
        
        shared_Pivot_Col[y][x + Half_BF] = (shared_Pivot_Col[y][x + Half_BF] > \
            shared_Pivot_Col[y][k] + shared_Pivot_Block[k][x + Half_BF]) ? \
                shared_Pivot_Col[y][k] + shared_Pivot_Block[k][x + Half_BF] : shared_Pivot_Col[y][x + Half_BF];

        shared_Pivot_Col[y + Half_BF][x] = (shared_Pivot_Col[y + Half_BF][x] > \
            shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Block[k][x]) ? \
                shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Block[k][x] : shared_Pivot_Col[y + Half_BF][x];
        
        shared_Pivot_Col[y + Half_BF][x + Half_BF] = (shared_Pivot_Col[y + Half_BF][x + Half_BF] > \
            shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Block[k][x + Half_BF]) ? \
                shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Block[k][x + Half_BF] : shared_Pivot_Col[y + Half_BF][x];

        // Calculate Pivot Row
        shared_Pivot_Row[y][x] = (shared_Pivot_Row[y][x] > \
            shared_Pivot_Block[y][k] + shared_Pivot_Row[k][x]) ? \
                shared_Pivot_Block[y][k] + shared_Pivot_Row[k][x] : shared_Pivot_Row[y][x];

        shared_Pivot_Row[y + Half_BF][x] = (shared_Pivot_Row[y + Half_BF][x] > \
            shared_Pivot_Block[y + Half_BF][k] + shared_Pivot_Row[k][x]) ? \
                shared_Pivot_Block[y + Half_BF][k] + shared_Pivot_Row[k][x] : shared_Pivot_Row[y + Half_BF][x];

        shared_Pivot_Row[y][x + Half_BF] = (shared_Pivot_Row[y][x + Half_BF] > \
            shared_Pivot_Block[y][k] + shared_Pivot_Row[k][x + Half_BF]) ? \
                shared_Pivot_Block[y][k] + shared_Pivot_Row[k][x + Half_BF] : shared_Pivot_Row[y][x + Half_BF];

        shared_Pivot_Row[y + Half_BF][x + Half_BF] = (shared_Pivot_Row[y + Half_BF][x + Half_BF] > \
            shared_Pivot_Block[y + Half_BF][k] + shared_Pivot_Row[k][x + Half_BF]) ? \
                shared_Pivot_Block[y + Half_BF][k] + shared_Pivot_Row[k][x + Half_BF] : shared_Pivot_Row[y + Half_BF][x + Half_BF];*/
...
__global__ void blockedFW_Phase3(int *D, unsigned int round, unsigned int V) {
    ...
    #pragma unroll 64
    for(int k = 0; k < BlockingFactor; k++) {
        /*data_0 = (data_0 >= (shared_Pivot_Col[y][k] + shared_Pivot_Row[k][x])) ? shared_Pivot_Col[y][k] + shared_Pivot_Row[k][x] : data_0;
        data_1 = (data_1 >= (shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Row[k][x])) ? shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Row[k][x] : data_1;
        data_2 = (data_2 >= (shared_Pivot_Col[y][k] + shared_Pivot_Row[k][x + Half_BF])) ? shared_Pivot_Col[y][k] + shared_Pivot_Row[k][x + Half_BF] : data_2;
        data_3 = (data_3 >= (shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Row[k][x + Half_BF])) ? shared_Pivot_Col[y + Half_BF][k] + shared_Pivot_Row[k][x + Half_BF] : data_3;*/
...
int main(int argc, char* argv[]) {
    ...
    //endSpan(t, totalIOTime);
	//printf("total IO time: %d (ms)\n", totalIOTime);
