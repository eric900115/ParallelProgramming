...
/*__global__ void blockedFW_Phase1(int *D, unsigned int round, unsigned int V) {
    ...
    // evaluate shared mem access offset
    int offset_1 = MEM_y_Offest;
    int offset_2 = 2 * MEM_y_Offest;
    int offset_3 = 3 * MEM_y_Offest;
    ...
    shared_D[y + offset_1][x] = D[(global_y + offset_1) * V + global_x];
    shared_D[y + offset_2][x] = D[(global_y + offset_2) * V + global_x];
    shared_D[y + offset_3][x] = D[(global_y + offset_3) * V + global_x];
    ...
    for(int i = 0; i < BlockingFactor; i++) {
        ...
        shared_D[y + offset_1][x] = min(shared_D[y + offset_1][x], shared_D[y + offset_1][i] + shared_D[i][x]);
        shared_D[y + offset_2][x] = min(shared_D[y + offset_2][x], shared_D[y + offset_2][i] + shared_D[i][x]);
        shared_D[y + offset_3][x] = min(shared_D[y + offset_3][x], shared_D[y + offset_3][i] + shared_D[i][x]);
        __syncthreads();
    }

    // store data from shared mem to global mem
    D[global_y * V + global_x] = shared_D[y][x];
    D[(global_y + offset_1) * V + global_x] = shared_D[y + offset_1][x];
    D[(global_y + offset_2) * V + global_x] = shared_D[y + offset_2][x];
    D[(global_y + offset_3) * V + global_x] = shared_D[y + offset_3][x];
