...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    __shared__ unsigned char sR_shared[9][137];
    __shared__ unsigned char sG_shared[9][137];
    __shared__ unsigned char sB_shared[9][137];


    if (bound_check(x, 0, width) && bound_check(y, 0, height)) {
        int global_id = channels * (width * (y) + (x));
        sR_shared[threadIdx.y][threadIdx.x] = s[global_id + 2];
        sG_shared[threadIdx.y][threadIdx.x] = s[global_id + 1];
        sB_shared[threadIdx.y][threadIdx.x] = s[global_id + 0];
    }

    __syncthreads();

    if (x >= width || y >= height || x < 0 || y < 0 \
        || threadIdx.x == 0 || threadIdx.x == 1 || threadIdx.x == (blockDim.x - 1) \
        || threadIdx.x == (blockDim.x - 2) || threadIdx.y == 0 || threadIdx.y == 1 \
        || threadIdx.y == (blockDim.y - 1) || threadIdx.y == (blockDim.y - 2)) return;
        ...
        for (int v = -yBound; v <= yBound; ++v) {
            ...
            for (int u = -xBound; u <= xBound; ++u) {
                if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                    const unsigned char R = sR_shared[threadIdx.y + v][threadIdx.x + u];//s[channels * (width * (y + v) + (x + u)) + 2];
                    const unsigned char G = sG_shared[threadIdx.y + v][threadIdx.x + u];//s[channels * (width * (y + v) + (x + u)) + 1];
                    const unsigned char B = sB_shared[threadIdx.y + v][threadIdx.x + u];//s[channels * (width * (y + v) + (x + u)) + 0];
...
int main(int argc, char **argv) {
    ...
    dim3 grid((width / 124 + 1), (height / 4 + 1), 1);
    dim3 block(128, 8, 1);
